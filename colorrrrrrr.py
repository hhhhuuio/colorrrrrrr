    ...
    def build_color_bar(colors, props, title, key):
        ...
        for c, p in zip(colors, props):
            ...
            fig.add_trace(go.Bar(
                ...
                hovertemplate=(
                    f"<span style='color:{hex_val};font-size:24px;'>██████</span><br>"
                    f"<b>{hex_val}</b><br>"
                    f"RGB: {rgb_val}<br>"
                    f"占比: {p*100:.2f}%<extra></extra>"
                ),
                showlegend=False,
                hoverlabel=dict(bgcolor="white", font_size=12, bordercolor="rgba(0,0,0,0.15)", namelength=0)
            ))
            start += p
        fig.update_layout(
            barmode='stack', height=56, margin=dict(l=0, r=0, t=0, b=0),
