...
def build_hue_wheel(colors_prop, props_prop, dominant_color, focus_main, wheel_size, uirev, dot_size=18):
    ...
    fig.update_layout(height=wheel_size, autosize=True, margin=dict(l=10,r=10,t=10,b=10),
...
if uploaded_file is not None:
    ...
    # ═══════════════════════════════════════════════════════════
    # 主体色环控制栏（两个独立复选框 + 聚焦主色 + 色点大小 + 恢复默认）
    # ═══════════════════════════════════════════════════════════
    st.markdown("### 色彩环视")
    
    # 第一行：两个复选框，都选即为都展示
    col_hue, col_val = st.columns(2)
    with col_hue:
        show_hue = st.checkbox("显示色相环", value=True)
    with col_val:
        show_value = st.checkbox("显示明度环", value=False)  # 默认只显示色相环，用户可按需开启
    
    # 确保至少选择一个环
    if not (show_hue or show_value):
        st.warning("至少需要选择一个环，已自动勾选“显示色相环”")
        show_hue = True
    
    # 第二行：聚焦主色、色点大小、恢复默认按钮
    ...
    # 动态调整色环大小：并列时适当缩小，单独显示时放大
    if show_hue and show_value:
        wheel_size = 480   # 两个环并排，每个宽度480
    else:
        wheel_size = 600   # 单个环，放大尺寸
    ...
    if show_hue and show_value:
        ...
        with col_h:
            ...
            st.plotly_chart(fig_hue, config={'displayModeBar': False}, use_container_width=True)
        with col_v:
            ...
            st.plotly_chart(fig_val, config={'displayModeBar': False}, use_container_width=True)
    elif show_hue:
        # 仅色相环
        fig_hue = build_hue_wheel(colors_prop, props_prop, dominant_color, focus_main, wheel_size, uirev, dot_size)
        st.plotly_chart(fig_hue, config={'displayModeBar': False}, use_container_width=True)
